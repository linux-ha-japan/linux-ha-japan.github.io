...
なお、CentOSにはPacemakerが同梱されているため、今まではyumでアップデートするとCentOSのパッケージで上書きされてしまうという事象があり、[インストール手順](/wp/dl/pminstall_cent5)に書いてあるように/etc/yum.repo.d/****.repoファイルに、excludeを記述する必要がありましたが、パッケージ名変更に伴いこの作業が不要となります。
