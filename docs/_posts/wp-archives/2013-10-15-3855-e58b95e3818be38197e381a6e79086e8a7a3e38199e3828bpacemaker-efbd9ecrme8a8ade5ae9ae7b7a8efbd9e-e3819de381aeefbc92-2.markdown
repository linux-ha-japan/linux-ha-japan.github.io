...
<td colspan="2" >
	primitive  _リソースID_  _RA名_
	    [meta  _リソースの動作設定_...]
	    [params  _RAに渡すパラメータ_...]
	    [op  start|stop|monitor  _オペレーション時の設定_...]
...
<td >リソースの名前を任意の英数字で定義します。クラスタ内で一意でなければなりません。CRMファイルの他の箇所でこのリソースを示す場合やcrm_monコマンドでの表示等にも使用します。
</td>
</tr>
<tr >
...
<td >リソースの故障時等の動作を個別に設定します。
	[前回ご紹介したrsc_defaultsコマンド](/wp/archives/3786)で指定できるresource-stickinessやmigration-thresholdパラメータを使用できます。
</td>
</tr>
<tr >
...
<td >RAがリソースの制御のためにパラメータを必要とする場合、「パラメータ名="値"」の形式でここで指定します。
    ...
    <code> # crm ra info ocf:heartbeat:Filesystem
     RAの簡易説明...</code>
...
開始および監視に失敗したときはリソース再起動、停止に失敗したときはクラスタのそれ以後の動作を中止します。
    ...
    <code>primitive resource1 ocf:heartbeat:Dummy \
        op start interval="0s" timeout="300s" on-fail="restart" \
        op monitor interval="10s" timeout="60s" on-fail="restart" \
        op stop interval="0s" timeout="300s" on-fail="block"
    </code>
...
<td colspan="2" >group  _グループID_  _リソースID_...
</td>
		</tr>
		<tr >
			設定項目
...
<td >グループの名前を任意の英数字で定義します。クラスタ内で一意でなければなりません。
</td>
		</tr>
		<tr >
...
<td >グループ化する対象リソースをprimitiveコマンドで定義したリソースIDで指定します。
</td>
		</tr>
	</tbody>
</table>
...
<td >クローンリソースの名前を任意の英数字で定義します。クラスタ内で一意でなければなりません。
</td>
		</tr>
		<tr >
...
<td >クローン化する対象リソースをprimitiveコマンドで定義したリソースIDで指定します。
</td>
		</tr>
		<tr >
...
次ページでこれらの動作を実験で確認します。
    ...
    独自のRAを作成する場合も、この仕様に準拠する必要があります。詳細は<a href="http://clusterlabs.org/doc/en-US/Pacemaker/1.0/html/Pacemaker_Explained/ap-lsb.html">公式マニュアル</a>を参照してください。
...
例のCRMに以下の設定を加えて、リソース(resource4)を追加します。
    ...
    <code>primitive resource4 ocf:heartbeat:Dummy \
        op start interval="0s" timeout="300s" on-fail="restart" \
        op monitor interval="10s" timeout="60s" on-fail="restart" \
        op stop interval="0s" timeout="300s" on-fail="block"
    </code>
...
**Pacemaker-1.0系の場合**
    ...
    <code># grep -e "lrmd.*rsc:.*start" /var/log/ha-log
...
**Pacemaker-1.1系の場合**
    ...
    <code># grep -e "Operation .*_start_" /var/log/ha-log
...
pm01(resource1が稼働しているノード)で以下コマンドを実行し、resource1を故障させます。
    ...
    <code>[root@pm01 ~]# rm -f /var/run/resource-agents/Dummy-resource1.state
    </code>
...
**Pacemaker-1.0系の場合**
    ...
    <code># grep -e "lrmd.*rsc:.*stop" /var/log/ha-log
...
**Pacemaker-1.1系の場合**
    ...
    <code># grep -e "Operation .*_stop_" /var/log/ha-log
